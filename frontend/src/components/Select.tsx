...
export function Select({
  ...
  return (
    <div className={clsx('block min-w-0 space-y-2', wrapperClassName)}>
      {label ? (
        <label htmlFor={selectId} className={clsx('text-sm font-medium text-slate-100', labelClassName)}>
          <span>{label}</span>
      ...
      <select
        ...
        className={clsx(
          'min-h-11 w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-teal-400/70 focus:bg-slate-950/90 focus-visible:ring-2 focus-visible:ring-teal-300/30 disabled:cursor-not-allowed disabled:border-slate-700/70 disabled:bg-slate-900/70 disabled:text-slate-400',
          isInvalid &&
