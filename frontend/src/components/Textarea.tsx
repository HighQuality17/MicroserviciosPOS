...
export function Textarea({
  ...
  return (
    <div className={clsx('block min-w-0 space-y-2', wrapperClassName)}>
      {label ? (
        <label htmlFor={textareaId} className={clsx('text-sm font-medium text-slate-200', labelClassName)}>
          <span>{label}</span>
      ...
      <textarea
        ...
        className={clsx(
          'min-h-28 w-full rounded-2xl border border-slate-700 bg-slate-950/70 px-4 py-3 text-sm text-slate-100 outline-none transition placeholder:text-slate-500 focus:border-teal-400/70 focus:bg-slate-950/90 focus-visible:ring-2 focus-visible:ring-teal-400/20 disabled:cursor-not-allowed disabled:border-slate-800 disabled:bg-slate-900/60 disabled:text-slate-500',
          isInvalid &&
